...
use anchor_spl::{associated_token::AssociatedToken, token_interface::{ mint_to, MintTo, Mint, TokenInterface, TokenAccount}};
use anchor_spl::metadata::{create_metadata_accounts_v3, create_master_edition_v3, sign_metadata, set_and_verify_sized_collection_item, SetAndVerifySizedCollectionItem, SignMetadata, CreateMasterEditionV3, CreateMetadataAccountsV3, Metadata, mpl_token_metadata::types::{Creator, CollectionDetails, DataV2}};
use Switchboard_on_demand::accounts::RandomnessAccountData;

declare_id!("GavVmR5a9jSTGhrQtAPPEmvNjUE52kYnGTwNPM4jd7Rt");
...
pub mod token_lottery {
    ...
    pub fn commit_randomness(ctx: Context<CommitRandomness>) -> Result<()> {
        ...
        let randomness_data = RandomnessAccountData::parse(ctx.accounts.randomness_account.data.borrow()).unwarp();
    ...
    pub fn reveal_winner(ctx: Context<RevealWinner>) -> Result<()> {
        ...
        let reveal_randomn_value = randomness_data.get_value(&clock)
        .map_err(|_| ErrorCode::RandomnessNotRevealed)?;

        let winner = reaveal_random_value[0] as u64 % token_lottery.total_tickets;
    ...
    pub fn claim_winnings(_ctx: Context<ClaimWinnings>) -> Result<()> {
        ...
        require!(ctx.accounts.ticket_metadata.collection.as_ref().unwrap().key == ctx.accounts.collection_mint.key(), ErrorCode::CollectionNotVerified);
...
pub struct Initialize<'info> {
    ...
    #[account(
        ...
        seeds = [b"collection_token".as_ref()],
        bump
    )]
