...
import SwicthboardIDL from '../switchboard.json';
...
describe('tokenlottery', () => {
 ...
 /* before("Load switchboard program", async () => {
    const switchboardIDL = await anchor.Program.fetchIdl(
      new anchor.web3.PublicKey("SWiTchBoaRd11111111111111111111111111111111"), // replace with actual program ID if different
      {connection: new anchor.web3.Connection("htpps://api.mainnet-beta.solana.com")}
    );
...
*/
  ...
  try {
    ...
    if (err instanceof Error && 'logs' in err) console.log("Logs:", (err as any).logs);
    throw err;
  }
}

  it('should test token lottery', { timeout: 300000 }, async () => {
    ...
    const initLotteryTx = new anchor.web3.Transaction(
      ...
      await buyTicket();
       ...
       const commitTx = new anchor.web3.Transaction(
        ...
        const reavealWinnerIx = await program.methods.revealWinner()
        .accounts({
          randomnessAccount: randomness.pubkey,
  }).instruction();
